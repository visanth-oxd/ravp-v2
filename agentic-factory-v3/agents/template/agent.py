"""
Template Agent – uses RegulatedAgent from SDK.

Copy this template to create a new agent.
Replace all "template" references with your agent name.
"""
...
AGENT_ID = "template"  # Change this to your agent ID


class TemplateAgent:
    """
    Template Agent using RegulatedAgent SDK.
    ...
    def process(self, input_data: str) -> dict:
        ...
        # Build system context from agent definition
        skills_text = ", ".join(self.regulated.skills) if self.regulated.skills else "general task handling"
        tools_text = ", ".join(self.regulated.allowed_tools)
        
        llm_prompt = f"""You are {self.regulated.agent_id}.

Mission: {self.regulated.purpose}

Your capabilities (skills): {skills_text}

Available tools to accomplish your skills: {tools_text}

---

Analyze this input and suggest action:

Input:
{input_data}

Based on this information and your capabilities, analyze and suggest the best action.
Respond with:
...
"""
        ...
        reasoning = self.regulated.llm.reason(llm_prompt, context={
            "input": input_data,
            "agent_id": self.regulated.agent_id,
            "purpose": self.regulated.purpose,
            "skills": self.regulated.skills
        })
    ...
    def answer(self, user_input: str) -> str | None:
        ...
        if lower == "help":
            help_text = (
                "Template Agent – interactive session.\n\n"
                "Commands:\n"
...
# Factory function
def create_agent(control_plane_url: str | None = None) -> TemplateAgent:
    """
    Factory function to create TemplateAgent instance.
    ...
    """
    return TemplateAgent(control_plane_url=control_plane_url)
